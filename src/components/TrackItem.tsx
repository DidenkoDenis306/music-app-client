...
function areEqual(prev: Props, next: Props) {
    return prev.track._id === next.track._id;
}

export const TrackItem: FC<Props> = memo(({track}) => {
    ...
    const handleCLickPlay = (event: any) => {
        ...
        if (isPlaying && track._id === activeTrack?._id) {
            pause();
        } else {
    ...
    const {mutate} = useMutation({
        mutationFn: () => axios.delete(`http://localhost:5555/tracks/${track._id}`),
    });
    ...
    return (
        ...
        /> */}
                ...
                <Box
                    ...
                    alignItems="center"
                    sx={{
                        opacity: activeTrack?._id === track._id ? 0.5 : 0,
